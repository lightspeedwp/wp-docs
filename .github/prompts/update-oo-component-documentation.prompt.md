...
Update the existing documentation file at: `${file}` by analysing the corresponding component code.

Extract the component path from the existing documentation's front matter (`component_path` field) or infer it from the documentation content. Analyse the current component implementation and update the documentation accordingly.
