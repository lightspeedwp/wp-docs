...
if (exists('AGENTS.md')) {
  ...
  requiredHeadings.forEach(h => { if (!content.includes(h)) errors.push(`AGENTS.md missing heading fragment: ${h}`); });
}
...
function validateAgentFile(relPath) {
  ...
  fragments.forEach(f => { if (!content.includes(f)) errors.push(`${relPath} missing section fragment: ${f}`); });
  // Encourage link back to global
  if (!content.includes('AGENTS.md')) warnings.push(`${relPath} should reference global AGENTS.md for inheritance.`);
  if (content.length > 12000) warnings.push(`${relPath} unusually large (>12KB) — consider refactoring.`);
}
...
if (exists('AGENTS.md') && detectFrontmatter('AGENTS.md')) {
  warnings.push('AGENTS.md has frontmatter; tagging enforcement not yet active.');
}
