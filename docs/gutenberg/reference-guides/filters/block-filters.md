...
WordPress exposes several APIs that allow you to modify the behaviour of existing blocks.
...
Here's a more robust example that disables background colour and gradient support for Heading blocks. The `block_type_metadata` filter is excellent for [curating the Editor experience](https://developer.wordpress.org/block-editor/how-to-guides/curating-the-editor-experience/). 
...
The following code will disable the colour controls for Paragraph, Heading, List, and List Item blocks.
...
Filters the front-end content of any block. This filter has no impact on the behaviour of blocks in the Editor. 
...
The following JavaScript filters are available to change the behaviour of blocks while editing in the Editor.
