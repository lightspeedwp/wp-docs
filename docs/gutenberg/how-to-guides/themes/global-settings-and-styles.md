...
-   What customisation options should be made available or hidden from the user.
-   What are the default colours, font sizes... available to the user.
...
- **Presets**: [colour palettes](/docs/how-to-guides/themes/theme-support.md#block-colour-palettes), [font sizes](/docs/how-to-guides/themes/theme-support.md#block-font-sizes), or [gradients](/docs/how-to-guides/themes/theme-support.md#block-gradient-presets) declared by the theme are converted to CSS Custom Properties and enqueued both the front-end and the editors.
...
| `editor-color-palette`      | Provide the list of colours via `color.palette`.               |
...
- `color.palette`:
    - generates 3 classes per preset value: colour, background-colour, and border-colour.
...
- Enable custom colours only for the paragraph block:
...
If you register a background colour for the root using styles.colour.background:
...
- Parsability, for machine understanding. Using a defined structure allows machines to understand the meaning of the property `--wp--preset--color--black`: it's a value bounded to the colour preset whose slug is "black", which then gives us room to do more things with them.
...
In WordPress 5.8, the CSS for some of the presets defined by WordPress (font sizes, colours, and gradients) was loaded twice for most themes: in the block-library stylesheet plus in the global stylesheet. Additionally, there were slight differences in the CSS in both places.
...
### Specificity for link colours provided by the user

In WordPress 5.8, when a user selected a link colour for a specific block we attached a class to that block in the form of `.wp-element-<ID>` and then enqueued the following style:

```css
.wp-element-<ID> a { color: <USER_COLOR_VALUE> !important; }
```

While this preserved user preferences at all times, the specificity was too strong and conflicted with some blocks with legit uses of an HTML element that shouldn’t be considered links. To [address this issue](https://github.com/WordPress/gutenberg/pull/34689), in WordPress 5.9 release, the `!important` was removed and updated the corresponding blocks to style the a elements with a specificity higher than the user link colour, which now is:

```css
.wp-element-<ID> a { color: <USER_COLOR_VALUE>; }
```

As a result of this change, it’s now the block author and theme author’s responsibility to make sure the user choices are respected at all times and that the link colour provided by the user (specificity 011) is not overridden.
