...
-   **Initializing the local context**
    ...
    You can also initialise the local context on the server using the `wp_interactivity_data_wp_context` PHP helper, which ensures proper escaping and formatting of the stringified values:
...
4. **Improved performance:** In many cases, derived state can be optimised to recalculate only when necessary, potentially improving your interactive blocks' performance.
