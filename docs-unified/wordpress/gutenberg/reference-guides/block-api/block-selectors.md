...
Block Selectors is the API that allows blocks to customise the CSS selector used
when their styles are generated.

A block may customise its CSS selectors at three levels: root, feature, and
subfeature.
...
Feature selectors relate to styles for a block support, e.g. border, colour,
typography, etc.

A block may wish to apply the styles for specific features to different
elements within a block. An example might be using colours on the block's wrapper
