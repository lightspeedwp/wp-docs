...
Let's start by creating the simplest Copyright Date Block possible, which will be a dynamically rendered block that simply displays the copyright symbol (©) and the current year. We'll also add a few controls allowing the user to modify font size and text colour.
...
Next, let's add a few [block supports](https://developer.wordpress.org/block-editor/reference-guides/block-api/block-supports/) so that the user can control the font size and text colour of the block.
...
Note that when you enable text colour support with `"text": true`, the background colour is also enabled by default. You are welcome to keep it enabled, but it's not required for this tutorial, so you can manually set `"background": false`.

Save the file and select the block in the Editor. You will now see both Colour and Typography panels in the Settings Panel. Try modifying the settings and see what happens.

![The block in the Editor with block supports](https://developer.wordpress.org/files/2023/12/block-tutorial-5.png)

#### Removing unnecessary code

For simplicity, the styling for the Copyright Date Block will be controlled entirely by the colour and typography block supports. This block also does not have any front-end JavaScript. Therefore, you don't need to specify stylesheets or a `viewScript` in the `block.json` file.
...
At this point, the block's icon and description are correct, and block supports allow you to change the font size and text colour. Now, it's time to move on to the actual functionality of the block.
...
Finally, make sure that there are no unsaved changes and then terminate the `npm run start` command. Run `npm run build` to optimise your code and make it production-ready.
...
Earlier in this tutorial, you added block supports that automatically created Colour and Typography panels in the Settings Sidebar of the block. You can create your own custom panels using the `InspectorControls` component.
...
<div class="callout callout-tip">
	<p>Three variables are exposed in <code>render.php</code>, which you can use to customise the block's output:</p>
	<ul>
...
The final step is to optimise the `render.php` file. If the `currentYear` and the `fallbackCurrentYear` attribute are the same, then there is no need to dynamically create the block content. It is already saved in the database and is available in the  `render.php` file via the `$content` variable.
